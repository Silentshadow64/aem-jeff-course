# Week 10 — Project (Author + Front-End)

**Theme:** Design Systems • Tokens & Reusable Patterns  
...
## Prerequisites
- You completed `week10/exercise.md` on a feature branch.
...
1) Build the target page(s)
   - Target: A page using **design tokens** via `theme.json` and a reusable pattern.
   - Insert required blocks exactly as specified in `week10/exercise.md`.
   - Save as **Draft** first.
...
1) Locate and adjust files
   - Open folder: Theme’s `theme.json` + pattern files
...
- Screenshots in `week10/screenshots/`:
  1. `week10_project_editor_<yourname>.png` — editor view  
  2. `week10_project_published_<yourname>.png` — published page  
  3. `week10_project_pr_<yourname>.png` — Pull Request page  
  4. `week10_project_structure_<yourname>.png` — file/folder view showing edited files
...
1) Branch
   - Name: `<yourname>/week-10-project`

2) Commit
   - Message: `Week 10 Project: <short description>`

3) Push and Pull Request
   - Title: `[<YourName>][Week10] Project Submission`
